...
1. Treat autonomous resolution as the first priority. Inspect, diagnose, install needed local tools, and repair the problem end to end before considering any blocked status.
2. Identify the failure signature and the capability difference between this dedicated recovery runner and the failed runner.
3. Avoid repeating failed actions without new information. Use host/control-plane authority when the ordinary worker was blocked by its isolation boundary, while preserving declared source and evidence protections.
4. Attempt targeted repair.
5. Run the smallest meaningful validation.
6. Write recovery evidence under `{{task_evidence_run_dir}}`.
7. Write `report.md`, `agent_status.json`, and command evidence. Use
   ...
   incomplete items.
8. Keep transient recovery evidence under the role output directory. Put durable
   project data in project paths only when it is part of the repaired
   deliverable.
9. Do not write `{{plan_file}}`, authoritative `validation.json`,
   ...
   by hand.
10. Run the commands needed to complete recovery end to end under the runner
   ...
   boundary.
11. If recovery starts background work and the next agent cannot safely
