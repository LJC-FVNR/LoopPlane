(function () {
  ...
  function nodeTiming(node) {
    ...
    return {
      ...
      ended: cleanText(item.ended_at) || (ended !== null ? timestampIso(ended) : (active && started !== null ? "running" : "pending")),
  ...
  function renderNodeDetailSection(section) {
    ...
    var title = text(item.title || key.replace(/_/g, " "));
    if (item.available !== true) {
      return '<section class="node-detail-section" data-section="' + escapeHtml(key) + '" data-available="false">' +
        "<h4>" + escapeHtml(title) + "</h4>" +
        '<p class="empty-state">' + escapeHtml(item.empty_message || "No evidence recorded.") + "</p>" +
        "</section>";
    }
    var body = "";
    if (item.path) {
  ...
  function renderPhaseTiming(timing, maxDuration) {
    ...
    return '<div class="phase-timing" data-phase-timing="' + escapeHtml(timing.active ? "running" : "recorded") + '">' +
      ...
      '<span><small>End</small><strong>' + escapeHtml(timing.end === null && timing.active ? "running" : compactTimestamp(timing.end)) + "</strong></span>" +
  ...
  function renderPlanPanel(planIndex, planMarkdown, workflowGraph, payload) {
    ...
    phases.forEach(function (phase, phaseIndex) {
      ...
      var taskRows = tasks.map(function (task) {
        ...
        return '<li class="task-row" ' + statusAttributes(status) + ">" +
          statusPill(status) +
          "<div>" +
      ...
      checklistBlocks.push('<article class="phase-block" ' + statusAttributes(phaseStatus) + ">" +
  ...
  function markdownAssetUrl(rawPath, context) {
    ...
    if (payload.server_mode && payload.workflow_id) {
      return fileUrl(payload, resolved);
    }
    var base = cleanText(payload.static_project_root_href || "../..");
    return joinUrlPath(base, resolved);
  }
  ...
  function markdownPlainText(value, context) {
    ...
    var output = [];
    var cursor = 0;
    var pattern = /(https?:\/\/[^\s<>()"']+|(?:\.loopplane|[A-Za-z0-9._-]+)\/[^\s<>()"']+)/g;
    var match;
    while ((match = pattern.exec(source))) {
      output.push(markdownEmphasis(source.slice(cursor, match.index)));
      var token = trimMarkdownLinkToken(match[0]);
      output.push(markdownAnchor(token.href, markdownLinkHref(token.href, context), "markdown-plain-link"));
      output.push(markdownEmphasis(token.trailing));
      cursor = match.index + match[0].length;
    }
    output.push(markdownEmphasis(source.slice(cursor)));
    return output.join("");
  }

  function trimMarkdownLinkToken(value) {
    var href = String(value || "");
    var trailing = "";
    while (href && /[.,;:]/.test(href.charAt(href.length - 1))) {
  ...
  function renderGraphPanel(workflowGraph, planIndex) {
    ...
    var phaseHtml = groups.map(function (group) {
      ...
      return '<section class="graph-phase-lane graph-phase-group" data-phase-key="' + escapeHtml(group.phase_key || "") + '" ' + statusAttributes(groupStatus) + ">" +
    ...
    return '<div class="graph-mode-toolbar">' +
      ...
      "</div>" +
      renderGraphOverview(groups, nodes, edges) +
  ...
  function renderGraphOverview(groups, nodes, edges) {
    ...
    return '<div class="graph-overview" aria-label="Graph summary">' +
      ...
      "<span><strong>" + escapeHtml(eventCount) + "</strong> events</span>" +
  ...
  function renderGraphTaskCard(task, nodes) {
    ...
    return '<article class="graph-task-card" ' + statusAttributes(taskStatus) + ' data-task-id="' + escapeHtml(taskId) + '">' +
  ...
  function graphNodeWithRelatedTime(agent, related) {
    ...
    copy.ended_at = copy.ended_at || new Date(Math.max.apply(Math, timestamps)).toISOString();
    return copy;
  }
  ...
  function renderFreshnessBanner(payload) {
    ...
    return '<section class="freshness-banner" data-freshness-status="' + escapeHtml(status) + '" role="status" aria-label="Read model freshness warning">' +
      "<div>" +
      '<p class="eyebrow">Freshness Warning</p>' +
      "<h2>" + escapeHtml(status === "stale" ? "Read Models May Be Stale" : "Read Model Freshness Needs Rebuild") + "</h2>" +
      "<p>" + escapeHtml(freshness.summary || "Read models require a rebuild before dashboard status can be trusted.") + "</p>" +
  ...
  function renderReadModelRebuildForm(payload, rebuild) {
    ...
    return '<form id="read-model-rebuild-form" class="rebuild-request-form" data-endpoint="' + escapeHtml(rebuild.endpoint || "") + '">' +
      ...
      '<button type="submit" data-rebuild-action="rebuild_read_models"' + disabledAttr + ">Create Rebuild Request</button>" +
      '<p id="read-model-rebuild-status" class="selector-status" role="status">' +
      escapeHtml(readModelRebuildStatusMessage(payload.server_mode, mutationAllowed, blockers)) + "</p>" +
      '<dl class="detail-list compact-detail-list">' +
  ...
  function readModelRebuildStatusMessage(serverMode, mutationAllowed, blockers) {
    if (!serverMode) {
    ...
    return "Creates a rebuild_read_models control request only; the runtime rebuild command applies derived read-model changes.";
  }
  ...
  function mountRefreshControls(payload) {
    ...
    setRefreshStatus("Auto-refresh every 30s.", "success");
    updateRefreshDisplay(payload, "");
    startAutoRefresh();
  }
  ...
  function refreshDashboard(source) {
    ...
    setRefreshStatus(source === "manual" ? "Refreshing now..." : "Auto-refreshing...", "active");
    fetchDashboardData(payload.workflow_id).then(function (nextPayload) {
      refreshInFlight = false;
      applyPayload(nextPayload, source === "manual" ? "Manual refresh complete." : "Auto refresh complete.");
  ...
  function fileUrl(payload, path, options) {
    ...
    var url = "/api/workflows/" + encodeURIComponent(payload.workflow_id) + "/files" +
      token + (token ? "&" : "?") + "path=" + encodeURIComponent(path);
  ...
  function streamLogTail(payload, button) {
    ...
    function refreshTail() {
      ...
      window.fetch(href, {credentials: "same-origin", headers: {"Accept": "text/plain"}}).then(function (response) {
        ...
        openFilePreview(title, path, "Unable to stream log: " + error.message, "", "text");
        stopLogStream();
      }).then(function () {
